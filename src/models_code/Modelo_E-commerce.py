...
def remove_colunas(lista_dataset: list[Dataset], colunas: list[str] = ["text", "group", "labels_int"]):
...
def treinamento(model, tokenizer, dataset_agrupado: list[Dataset], optimizer, num_epochs: int, num_batchs: int, num_class: int) -> None:
    try:
        ...
        precision = func_precision(dataset_agrupado[-1]["labels"], y_pred, num_class)
        recall = func_recall(dataset_agrupado[-1]["labels"], y_pred, num_class)
        f1 = func_f1(dataset_agrupado[-1]["labels"], y_pred, num_class)
